...
func TestRoutingTable_AddPeer_Success(t *testing.T) {
	...
	remoteID[31] = 0x01
	peer, _ := domain.NewPeer(remoteID, []string{"10.0.0.2:9000"}, nil)
...
func TestRoutingTable_AddPeer_RejectSelf(t *testing.T) {
	...
	selfPeer, _ := domain.NewPeer(localID, []string{"10.0.0.1:9000"}, nil)
...
func TestRoutingTable_BucketFull(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		peer, _ := domain.NewPeer(remoteID, []string{"10.0.0.x:9000"}, nil)
...
func TestRoutingTable_ClosestPeers(t *testing.T) {
	...
	p1, _ := domain.NewPeer(p1ID, []string{"10.0.0.1:9000"}, nil)
	p2, _ := domain.NewPeer(p2ID, []string{"10.0.0.2:9000"}, nil)
	p3, _ := domain.NewPeer(p3ID, []string{"10.0.0.3:9000"}, nil)
